...
function findMissingWorkflows(node) {
    ...
    return (node.manifest.text.services ?? []).flatMap((s) =>
        [...(s.automated ?? []), ...(s.outOfBand ?? [])]
            ...
            .map((g) => ({ ...rest, ...g, file: g.file.replace(".github/workflows/", "") }))
    );
}
...
function getQualityGatesForRepo(name) {
    const node = nodesWithManifest.find((n) => n.name === name);
    return (node?.manifest.text.services ?? []).flatMap((s) =>
        [...(s.automated ?? []), ...(s.outOfBand ?? [])].map((g) => ({ "product": s.product, ...g, name: node.name, "pod.value": node.pod?.value, "teamResponsible.value": node.teamResponsible?.value }))
    );
}
```
...
Inputs.table(
    ...
    { columns: ["name", "pod.value", "teamResponsible.value", "product", "phase", "provider", "checkTypes", "file", "path"] })
```
...
function findGatesWithmismatchedJobs(node) {
    ...
    return (node.manifest.text.services ?? []).flatMap((s) =>
        [...(s.automated ?? []), ...(s.outOfBand ?? [])]
            .filter((g) => g.provider === "GitHub" && g.path)
            .flatMap((g) => {
...
function findGatesWithInvalidPaths(node) {
    ...
    return (node.manifest.text.services ?? []).flatMap((s) =>
        [...(s.automated ?? []), ...(s.outOfBand ?? [])]
            .filter((g) => g.provider === "GitHub" && g.path && !parseCheckPath(g.path).valid)
            .map((g) => ({ ...rest, ...g, invalidPath: g.path }))
    );
}
