...
describe("parseCheckPath", () => {
  describe("standard $ prefixed paths", () => {
    ...
    it("parses underscored job names", () => {
      assert.deepEqual(parseCheckPath("$.jobs.run_tests"), { valid: true, job: "run_tests", step: null });
    });
  });

  describe("legacy paths without $ prefix", () => {
    it("parses legacy dot-notation job path", () => {
      assert.deepEqual(parseCheckPath("jobs.build"), { valid: true, job: "build", step: null });
    });

    it("parses legacy hyphenated job path", () => {
      assert.deepEqual(parseCheckPath("jobs.integration-test"), { valid: true, job: "integration-test", step: null });
    });

    it("parses legacy path with step filter by name", () => {
      assert.deepEqual(parseCheckPath("jobs.build.steps[?@.name=='Run tests']"), {
        valid: true, job: "build", step: { by: "name", value: "Run tests" },
      });
    });

    it("parses legacy path with step filter by id", () => {
      assert.deepEqual(parseCheckPath("jobs.test.steps[?@.id=='lint']"), {
        valid: true, job: "test", step: { by: "id", value: "lint" },
      });
    });
  });
  ...
  describe("invalid paths", () => {
    ...
    it("returns invalid for empty string", () => {
      assert.deepEqual(parseCheckPath(""), { valid: false });
    });
    ...
    it("returns invalid for filter-only paths", () => {
      assert.deepEqual(parseCheckPath("$[?@.ParameterKey=='X']"), { valid: false });
    });

    it("returns invalid for bare job name without jobs prefix", () => {
      assert.deepEqual(parseCheckPath("build"), { valid: false });
    });
  });
});
...
describe("isValidJsonPath", () => {
  ...
  it("returns false for legacy paths without $", () => {
