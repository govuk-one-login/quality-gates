...
describe("upgrade command", () => {
  it("upgrades a v0.1.0 manifest in place", () => {
    ...
    assert.match(result.$schema, /v0\.9\.0/);
  });
  ...
  it("skips manifests with local $schema", () => {
    const file = createManifest(TMP, {
      ...
      services: [{ serviceTag: "x", promotionType: "library", qualityGates: [] }],
    });
    const before = readFileSync(file, "utf8");
  ...
  it("skips manifests already at latest version", () => {
    const file = createManifest(TMP, {
      $schema: "https://raw.githubusercontent.com/govuk-one-login/quality-gates/refs/tags/v0.9.0/schemas/schema.json",
      services: [{ serviceTag: "x", promotionType: "securePipelines", qualityGates: [] }],
    });
    const before = readFileSync(file, "utf8");
  ...
  it("finds manifests recursively in a directory", () => {
    ...
    assert.match(a.$schema, /v0\.9\.0/);
    assert.match(b.$schema, /v0\.9\.0/);
